...
test.describe("Library dialogs", () => {
  ...
  test("ConfirmDialog: opens with title and count in message", async ({
    page,
  }) => {
    ...
    await expect(dialog.getByTestId("confirm-dialog-message")).toContainText(
      "2 book(s)",
    );
  });

  test("ConfirmDialog: Cancel closes dialog and leaves books", async ({
    page,
  }) => {
    await page.getByTestId("delete-selected-button").click();
  ...
  test("ConfirmDialog: Confirm closes dialog and removes selected books", async ({
    page,
  }) => {
    await page.getByTestId("delete-selected-button").click();
  ...
  test("ConfirmDialog: Escape closes dialog and leaves books", async ({
    page,
  }) => {
    await page.getByTestId("delete-selected-button").click();
  ...
  test("MoveFolderDialog: opens with heading and renders root folder", async ({
    page,
  }) => {
  ...
  test("MoveFolderDialog: Cancel closes dialog and leaves books in place", async ({
    page,
  }) => {
    await page.getByTestId("move-selected-button").click();
  ...
  test("MoveFolderDialog: Escape closes dialog and leaves books in place", async ({
    page,
  }) => {
    await page.getByTestId("move-selected-button").click();
  ...
  test("MoveFolderDialog: Create new folder opens nested CreateFolderDialog", async ({
    page,
  }) => {
    await page.getByTestId("move-selected-button").click();
  ...
  test("MoveFolderDialog: Confirm with newly created folder moves books into it", async ({
    page,
  }) => {
    await page.getByTestId("move-selected-button").click();
    await page.getByTestId("create-new-folder-button").first().click();
    ...
    await expect(page.getByTestId("move-to-preview")).toContainText(
      "Move to: /Archive",
    );
    ...
    await expect(
      page.locator("details > summary", { hasText: "Archive" }),
    ).toBeVisible();
    await expect(page.getByTestId("book-checkbox")).toHaveCount(2);
  });
  ...
  test("CreateFolderDialog: opens with heading and focuses input", async ({
    page,
  }) => {
    await openCreateFolderDialog(page);
    await expect(
      page.getByTestId("create-folder-dialog").locator("h3"),
    ).toHaveText("Create New Folder");
    await expect(page.getByTestId("create-folder-input")).toBeFocused();
  });

  test("CreateFolderDialog: submit is disabled empty, enabled with text", async ({
    page,
  }) => {
    await openCreateFolderDialog(page);
  ...
  test("CreateFolderDialog: Enter creates folder, closes, and auto-selects it", async ({
    page,
  }) => {
    await openCreateFolderDialog(page);
    ...
    await expect(
      page.locator(
        '[data-testid="folder-button"][data-folder-path="Mythology"]',
      ),
    ).toBeVisible();
    await expect(page.getByTestId("move-to-preview")).toContainText(
      "Move to: /Mythology",
  ...
  test("CreateFolderDialog: Escape closes without creating", async ({
    page,
  }) => {
    await openCreateFolderDialog(page);
    ...
    await expect(
      page.locator(
        '[data-testid="folder-button"][data-folder-path="NeverCreated"]',
      ),
    ).toHaveCount(0);
  });

  test("CreateFolderDialog: Cancel closes without creating", async ({
    page,
  }) => {
    await openCreateFolderDialog(page);
    await page.getByTestId("create-folder-input").fill("NeverCreated");
