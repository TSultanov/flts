  ...
  const res = await page.evaluate(
    async ({ cmd, args, timeoutMs }) => {
      ...
      try {
        ...
        return {
          status: "rejected",
          value: String(JSON.stringify(err)).slice(0, 300),
        };
      } finally {
        clearTimeout(timer);
      }
...
test.describe("bridge conformance", () => {
  ...
  test("read-only commands settle", async ({ page }) => {
    ...
    const table: Array<[string, Record<string, unknown>?, number?]> = [
      ...
      [
        "get_system_definition",
        { word: "Hund", sourceLang: "deu", targetLang: "eng" },
      ],
  ...
  test("entity-dependent commands settle", async ({ page }) => {
    ...
    const table: Array<[string, Record<string, unknown>?, number?]> = [
      ...
      [
        "save_book_reading_state",
        { bookId, chapterId, paragraphId, pageOffset: 0 },
      ],
  ...
  test("mutating commands settle", async ({ page }) => {
    ...
    const table: Array<[string, Record<string, unknown>?, number?]> = [
      ...
      [
        "sync_add_device",
        { deviceId: "AAAAAAA-BBBBBBB-CCCCCCC-DDDDDDD", name: "peer" },
      ],
  ...
  test("every bridged command was covered", async () => {
    ...
    expect(
      [...covered].filter((c) => !ALL_COMMANDS.includes(c as never)),
    ).toEqual([]);
    ...
    const declared = [...block![1].matchAll(/"([a-z0-9_]+)"/g)].map(
      (m) => m[1],
    );
