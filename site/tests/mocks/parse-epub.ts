...
function textBetweenAnchors(
  ...
  const startElement =
    (anchor1 !== "" ? document.getElementById(anchor1) : document.body) ??
    document.body;
...
function textBetween(start: Element, end: Element | null): Paragraph[] {
  ...
  while (current && current !== end) {
    if (current.nodeType === Node.ELEMENT_NODE) {
      ...
      if (
        (!element.hasChildNodes() || allChildrenAreInline(element)) &&
        element.textContent &&
    ...
    if (
      ...
      !allChildrenAreInline(current as Element)
    ) {
...
export function getSanitizedHtml(element: Element, keepBoundingTag = true) {
  if (
    keepBoundingTag &&
    !allowListedTags.includes(element.tagName.toLowerCase())
  ) {
