...
export function buildLibraryFolder(
  books: LibraryBookMetadataView[],
): LibraryFolder {
...
export class Library {
  ...
  getWordInfo(
    ...
    sentenceId: number,
    wordId: number,
  ): Resource<SentenceWordTranslation | undefined> {
  ...
  getSystemDefinition(
    ...
    sourceLang: string,
    targetLang: string,
  ): Resource<SystemDefinition | null> {
  ...
  async translateParagraph(
    ...
    return await invoke<number>("translate_paragraph", {
      bookId,
      paragraphId,
  ...
  async translateChapter(
    ...
    let config = await getConfig();
    return await invoke<number>("translate_chapter", {
      bookId,
      chapterId,
      model: model ?? config.model,
      useCache,
  ...
  getParagraphTranslationActivity(
    bookId: UUID,
    paragraphId: number,
  ): ParagraphTranslationActivityResource {
  ...
  async getParagraphOriginalsBatch(
    ...
    return await invoke<ParagraphOriginal[]>("get_paragraph_originals_batch", {
      bookId,
      paragraphIds,
    });
  }
  ...
  async getBookReadingState(bookId: UUID): Promise<BookReadingState | null> {
    return await invoke<BookReadingState | null>("get_book_reading_state", {
      bookId,
    });
  }
