...
export type TranslationProvider = 'google' | 'openai' | 'deepseek' | 'zai' | 'openrouter';
...
export function openRouterFamilies(models: Model[]): string[] {
    const families = new Set(models.map((m) => openRouterFamilyFromModelId(m.id)));
    return [...families].sort((a, b) =>
        formatOpenRouterFamilyLabel(a).localeCompare(formatOpenRouterFamilyLabel(b)),
    );
}

export function openRouterModelsInFamily(models: Model[], family: string): Model[] {
