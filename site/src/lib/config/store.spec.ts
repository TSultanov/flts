...
describe("Config type", () => {
  ...
  it("accepts tapToRevealTranslations as an optional boolean", () => {
    ...
    expectTypeOf(withFlag.tapToRevealTranslations).toEqualTypeOf<
      boolean | undefined
    >();
...
describe("modelsForDropdown", () => {
  it("keeps a saved id missing from the catalog", () => {
    ...
    const { list, orphan } = modelsForDropdown(
      models,
      "google",
...
describe("OpenRouter family helpers", () => {
  ...
  it("extracts family prefix before slash", () => {
    expect(
      openRouterFamilyFromModelId("~deepseek/deepseek-v4-flash-latest"),
    ).toBe("deepseek");
    expect(openRouterFamilyFromModelId("deepseek/deepseek-v4-pro")).toBe(
      "deepseek",
    );
  ...
  it("lists sorted unique families", () => {
    expect(openRouterFamilies(models)).toEqual([
      "deepseek",
      "meta-llama",
      "other",
    ]);
  ...
  it("falls back to first family when nothing matches", () => {
    expect(
      resolveOpenRouterFamily("missing", "unknown", ["deepseek", "meta-llama"]),
    ).toBe("deepseek");
  });
