...
The `RefactoredGetTransHold` and `RefactoredGetTransSecond` actions in the spec model a _hypothetical_ refactoring where `get_or_create_translation` holds the translation lock across both the existence check and the insert. These actions are NOT instrumented from real code — they are spec-only fault injection.
