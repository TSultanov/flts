...
2. **Roster mesh (new):** `<library_root>/.flts/devices.json` is itself synced
   content. Every node lists _all_ mesh devices; each node reconciles its Syncthing
...
**Concurrency model:** async Rust (tokio). Three _independent control loops_ per node
...
**Key deviation from "just use Syncthing":** removal is _opt-in_ — the roster carries
...
but REMOVES an engine device only if the roster _tombstones it and does not re-list it_.
...
the folder with the newly-learned peer. The _first_ link still requires a two-sided
...
- Code analysis:
  ...
  - `sync.rs:127-165` — pairing is one-sided `pair_device`; the other half is the peer approving a _pending_ device (`pending_devices`), which the daemon does **not** auto-approve.
...
- Granularity: model replication as an asynchronous channel that can lag arbitrarily; this is where the _liveness_ question lives.
...
mtime. FLTS resolves those siblings per object type with _different_ rules — and "newest"
